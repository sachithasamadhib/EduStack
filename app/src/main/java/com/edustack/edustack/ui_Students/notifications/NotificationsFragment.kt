...
class NotificationsFragment : Fragment() {
    ...
    private fun loadNotifications() {
        ...
        // Firebase integration: Load notifications for the current student
        val repository = StudentRepository()
        lifecycleScope.launch {
            val notifications = repository.getNotificationsForCurrentStudent()
            val studentNotifications = notifications.filter { it.type == "STUDENT" }
            notificationsList.clear()
            notificationsList.addAll(studentNotifications)
            notificationsAdapter.updateNotifications(notificationsList)
            updateEmptyState()
